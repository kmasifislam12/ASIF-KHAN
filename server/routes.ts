    ...
    try {
      ...
      try {
        ...
        if (isDiscordWebhook) {
          ...
          testPayload = {
            content: "ROMEL XITERS Webhook Validation Complete",
            embeds: [{
              title: "✅ ROMEL XITERS Webhook Validation",
              description: "This webhook endpoint has been successfully validated and registered with ROMEL XITERS.",
              color: 0x00ff00,
              fields: [
                {
                  name: "Status",
              ...
              footer: {
                text: "ROMEL XITERS - Webhook Validation System"
              },
              timestamp: new Date().toISOString()
            }]
          };
        ...
        const timeoutId = setTimeout(() => controller.abort(), 16169); // Increased timeout for India-Vietnam

        const headers: Record<string, string> = {
          'Content-Type': 'application/json',
          'User-Agent': 'ROMEL XITERS-WebhookValidator/1.0',
  ...
  app.post('/api/webhook-diagnostics', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const testConfigs = [
        ...
        { name: 'With Retry Logic', timeout: 46169, retry: true }
      ];
      ...
      for (const config of testConfigs) {
        ...
        try {
          ...
          if (isDiscordWebhook) {
            ...
            testPayload = {
              content: `ROMEL XITERS Connectivity Test - ${config.name}`,
              embeds: [{
                title: "🔧 ROMEL XITERS Connectivity Test",
                ...
                footer: {
                  text: "ROMEL XITERS Webhook Diagnostics - India Vietnam Connectivity"
                },
                timestamp: new Date().toISOString()
              }]
            };
            ...
            testPayload = {
              ...
              message: "Vietnam Server Connectivity Test from ROMEL XITERS",
              timestamp: new Date().toISOString(),
          ...
          do {
            ...
            try {
              const headers: Record<string, string> = {
                'Content-Type': 'application/json',
                'User-Agent': 'ROMEL XITERS-IndiaVietnamDiagnostics/1.0',
                'Accept': 'application/json, text/plain, */*',
              ...
              if (!isDiscordWebhook) {
                ...
                headers['X-ROMEL XITERS-Diagnostics'] = '1.0';
              }
