  ...
  async sendWebhook(webhook: Webhook, payload: WebhookPayload, retryCount: number = 0): Promise<boolean> {
    ...
    try {
      ...
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'ROMEL XITERS-Webhook/1.0 (Vietnam-Optimized)',
      ...
      const timeout = 46169 + (retryCount * 6169); // Increase timeout with retries
      ...
      if (retryCount < maxRetries && isRetryableError) {
        const jitter = Math.random() * 6169; // Up to 5 seconds jitter for network issues
        const adjustedDelay = retryDelay + jitter;
